...
async function puppeteerCrawl(baseURL, data, callback) {
    console.log(`Crawling Stats for ${baseURL}`);
    ...
    try {
        await page.waitForSelector(".empty-state__message-label");
    ...
    // ABORT THESE REQUESTS FROM ALL PAGES BELOW THIS POINT
    ...
    data.players.forEach((player, index) => {
        player.fantasyAvg = playerSeasonStats[index];
        player.teamName = player.team == 0 ? matchup["Home"] : matchup["Away"];
        player.teamScore = player.team == 0 ? score["HomeScore"] : score["AwayScore"];
        player.teamScoreTotal = player.team == 0 ? score["HomeScoreTotal"] : score["AwayScoreTotal"];
        player.time = gameTime["Time"];
        if (subs["Subs"].includes(player.name.replace(/\./g, ' ').split(' ').reduce((shortName, word, index) => index === 0 ? `${word.charAt(0)}.` : shortName + word, '').toUpperCase())) {
            player.sub = 1;
        }
        // if a player is benched, set benched flag to 1
        if (benched["Benched"].includes(player.name)) {
            player.benched = 1;
        }
        if (benched["Injured"].includes(player.name)) {
            player.injured = 1;
        }
    });
    ...
    data.players.forEach((player, index) => {
        player.breakeven = breakevens[`${player.name}-${player.fantasyAvg}`];
    });
...
// Example usage:
// puppeteerCrawl(baseURL, data, (finalData) => {
//     // Do something with the final data, such as logging it
